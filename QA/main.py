...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='../data/Train_qa_preprocess.csv')
    parser.add_argument('--model_file', type=str, default='./qa_model.pt', help='Model file name.')
...
def train(args):
    ...
    train_input_ids, train_attention_masks, train_labels = tokenize(df['qa'], MAX_LENGTH, tokenizer, df['label'])
    ...
    for _ in range(EPOCHS):
        ...
        if accuracy_score(pred_tags, valid_tags)>max(valid_acc):
        ...
        elif accuracy_score(pred_tags, valid_tags)==max(valid_acc) and eval_loss<min_lss: 
            max_acc = accuracy_score(pred_tags, valid_tags)
            min_lss = eval_loss
            patient = 0
            print("saving state dict")
            ...
            patient += 1
            if patient == PATIENT:
                print(f'Early Stop. Best Acc {max_acc}, Best Lss {min_lss}')
                break
...
def evaluate(args):
    ...
    test_input_ids, test_attention_masks = tokenize(df['qa'], MAX_LENGTH, tokenizer)
    ...
    # output result 
    result = pd.DataFrame({'id':df['id'].values,'answer':predictions})

    return result.to_csv('qa.csv', index=None)
